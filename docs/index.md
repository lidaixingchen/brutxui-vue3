...
### CLI 工具链（CLI）
- [CLI代码修改与配置注入深模块重构方案](plans/cli/CLI代码修改与配置注入深模块重构方案.md)（状态：`active`）  
  *收敛分散在 init-service、tailwind-rules、vscode-snippets 中的标记块注入、Nuxt 状态机解析与 Snippets 合并，构建单一高内聚、幂等的 CodeModifier 深模块。*
...
- **[CLI 领域（14 篇）](archive/2026/cli/)**：CLI基础设施闭环、CLI声明式诊断巡检与自愈引擎、CLI样式自动生成与单一信源治理、CLI注册表深模块客户端演进重构、CLI注册表深模块客户端重构、CLI组件三方合并与代码升级引擎、CLI网络韧性与多源竞速自适应退避、CLI诊断引擎开放化与CI原生支持、CLI语法树解析升级与Monorepo工作区感知、CLI项目上下文与路径解析引擎封装、CLI项目上下文深模块凝聚与辅助函数收敛、Tailwind模块化依赖图扫描与样式解耦、registry产物发布时构建、CLI诊断自愈纯声明式方案与差异预览重构等。
...
- **[核心架构与基建（20 篇）](archive/2026/core/)**：AST解析统一与源码工具链治理、changelog自动化、composables状态只读化、代码质量与性能改进、元数据脚手架树模型与层级体系治理、全工程虚拟文件系统统一与持久化深模块重构、共享常量收割与构建校验防漂移、工程门禁与巡检脚本工具链重构、文档目录改造、文档链接检查与自愈引擎改进、方案自动归档与知识地图自愈引擎、架构优化方案-v1、架构优化方案-v2、架构优化方案-v3、死代码与动效预设清理、注册表编译与AST静态转换管线模块化、约定体系修复、编译扫描排除清单与覆盖规则下沉、辅助包改进方案-v1、辅助包改进方案-v2等。
