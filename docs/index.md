...
### 全局核心架构（Core）
- [工程门禁与巡检调度引擎收敛方案](plans/core/工程门禁与巡检调度引擎收敛方案.md)（状态：`active`）  
  *抽象门禁调度引擎深模块，收敛 Result Envelope 协议解析与子进程派发，降低 Windows 平台冷启动开销。*
...
- **[UI 组件领域（11 篇）](archive/2026/ui/)**：NumberInput视觉优化、命令式宿主控制器与弹层调度栈重构、命令式弹层宿主深化与MessageBox解耦、工控窗口交互升级与开关质感重塑、按压反馈盖影、滑块与滚动条工控实体质感重塑、组件拓展、组件深化与拓展、组件视觉效果深化与质感进阶、组件设计规范与视觉效果优化、组件选中态统一与交互无障碍补齐等。
- **[样式与设计系统（6 篇）](archive/2026/styles/)**：Tailwind颜色双轨与工具函数单一信源治理、主题系统三套合一与色彩对比度治理、状态生命周期色彩与组件双轨治理、阴影组装化重构、阴影过渡与焦点体系统一、跨包令牌编译器下沉与构建拓扑解耦等。
