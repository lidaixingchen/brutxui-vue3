...
- **[核心架构与基建（25 篇）](archive/2026/core/)**：AST解析统一与源码工具链治理、changelog自动化、composables状态只读化、代码质量与性能改进、元数据脚手架树模型与层级体系治理、全工程虚拟文件系统统一与持久化深模块重构、共享常量收割与构建校验防漂移、工程门禁与巡检脚本工具链重构、工程门禁与巡检调度引擎收敛、文档目录改造、文档链接检查与自愈引擎改进、方案自动归档与知识地图自愈引擎、架构交付契约第二批修复、架构交付契约首批修复、架构优化方案-v1、架构优化方案-v2、架构优化方案-v3、死代码与动效预设清理、注册表编译与AST静态转换管线模块化、系统演进与存量任务收敛、约定体系修复、编译扫描排除清单与覆盖规则下沉、辅助包改进方案-v1、辅助包改进方案-v2、架构交付契约第三批修复等。
