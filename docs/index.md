...
- [工程门禁与巡检脚本工具链重构方案](plans/core/工程门禁与巡检脚本工具链重构方案.md)（状态：`active`）  
  *系统性重构全仓自检与契约工具链，消除硬编码黑名单与 4 空格正则反查，重构 Markdown 计数型围栏状态机，统一仓库根路径推导与跨平台真实大小写核验，全面对齐 Agent Result Envelope 结构化输出。*
