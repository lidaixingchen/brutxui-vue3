...
- [CLI注册表深模块客户端演进重构方案](plans/cli/CLI注册表深模块客户端演进重构方案.md)（状态：`active`）  
  *收拢注册表上帝模块与周边浅包装集群，封装单一高内聚 RegistryClient，消除模块级全局状态污染，提供 DAG 依赖拓扑排序与极简调用 Seam。*
- [CLI项目上下文深模块凝聚与辅助函数收敛方案](plans/cli/CLI项目上下文深模块凝聚与辅助函数收敛方案.md)（状态：`active`）  
  *消除 project.ts 与 project-context.ts 的双重实现，将组件扫描与 SFC 转换收敛至 ProjectContext 单一聚合根，避免循环代理与重复实例化 I/O 开销。*
...
- **[CLI 领域（11 篇）](archive/2026/cli/)**：CLI基础设施闭环、CLI声明式诊断巡检与自愈引擎、CLI样式自动生成与单一信源治理、CLI注册表深模块客户端重构、CLI组件三方合并与代码升级引擎、CLI网络韧性与多源竞速自适应退避、CLI诊断引擎开放化与CI原生支持、CLI语法树解析升级与Monorepo工作区感知、CLI项目上下文与路径解析引擎封装、Tailwind模块化依赖图扫描与样式解耦、registry产物发布时构建等。
- **[UI 组件领域（10 篇）](archive/2026/ui/)**：NumberInput视觉优化、命令式弹层宿主深化与MessageBox解耦、工控窗口交互升级与开关质感重塑、按压反馈盖影、滑块与滚动条工控实体质感重塑、组件拓展、组件深化与拓展、组件视觉效果深化与质感进阶、组件设计规范与视觉效果优化、组件选中态统一与交互无障碍补齐等。
- **[样式与设计系统（5 篇）](archive/2026/styles/)**：Tailwind颜色双轨与工具函数单一信源治理、主题系统三套合一与色彩对比度治理、状态生命周期色彩与组件双轨治理、阴影组装化重构、阴影过渡与焦点体系统一等。
- **[核心架构与基建（20 篇）](archive/2026/core/)**：AST解析统一与源码工具链治理、changelog自动化、composables状态只读化、代码质量与性能改进、元数据脚手架树模型与层级体系治理、全工程虚拟文件系统统一与持久化深模块重构、共享常量收割与构建校验防漂移、工程门禁与巡检脚本工具链重构、文档目录改造、文档链接检查与自愈引擎改进、架构优化方案-v1、架构优化方案-v2、架构优化方案-v3、死代码与动效预设清理、注册表编译与AST静态转换管线模块化、约定体系修复、编译扫描排除清单与覆盖规则下沉、辅助包改进方案-v1、辅助包改进方案-v2、方案自动归档与知识地图自愈引擎等。
