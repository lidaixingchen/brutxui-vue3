...
在 [RELEASE.md](./RELEASE.md#breaking-change-迁移文档规范) 中规范化 breaking change 迁移文档模板：

- 每个 breaking change commit 必须带 `!` 标记或 `BREAKING CHANGE:` body
- 必须按模板提供「影响范围 / 变更原因 / 迁移步骤（before/after 代码）/ 自动迁移可行性」四段
- "自动迁移可行性" 字段用于累积 codemod 候选清单

**启动条件**

当真实 breaking change 累积到 3 个以上、且 release notes 中评估为「需要 codemod（涉及类型推断/复杂重构，无法靠文本替换）」的 case ≥ 3 个时，启动本 Item。在到达该阈值前，文档化替代已能覆盖 80% 场景。

### 10. Monorepo 工作区一等公民支持

**状态**：暂缓（替代方案已落地，倾向永久搁置）
...
BrutxUI 是 copy-paste 式组件库（核心理念见 [README-en.md](../docs/README-en.md)「copy-paste first」），把组件源码复制到每个子包与 monorepo 复用精神部分冲突。`shadcn/ui` 同类产品也不一等支持 monorepo——这是该品类的共识。

**轻量替代（已落地）**

[doctor.ts](../packages/cli/src/commands/doctor.ts) 新增 `checkWorkspaceHint` 检查：

- 在 monorepo 子包内运行 `doctor` 时输出 `warn` 提示
- 引导用户在各子包独立运行 `brutx-vue init`/`add`
- 共享依赖仍通过 `pnpm-workspace.yaml` 管理

**启动条件**

仅当出现真实 monorepo 用户反馈或企业版需求时再启动。在到达前，doctor 提示已能引导用户正确使用，避免本 Item 的复杂度（workspace 检测/子包枚举/依赖去重/manifest 聚合/版本冲突处理）提前落地。
