...
依赖 Item 4（构建增量）成熟后再做，watch 复用增量逻辑。
...
| 第 6 阶段 | 长期演进 | Item 9/10/11 | 按需求驱动 |
...
- **版本化去重必须按 (name, source) 键**：`resolveDeps` 的 `visited` 若仍按 `cleanName` 单键去重，版本化依赖会静默丢失，这是版本契约的前置正确性 bug。
