...
经 2026-07-08 审计，三条建议中两条已落地，剩余一条无具体消费方驱动：

1. **shared 元数据扩展** — `category` 已在 [component-registry.ts](../packages/shared/src/component-registry.ts) 通过 `CATEGORY_OVERRIDES` + `inferCategory()` 提供；`status`（`stable|legacy|deprecated`）+ `replacement` 已在 [types.ts](../packages/shared/src/types.ts) 定义。`tags`/`stability`（数值等级）/`experimental` 未引入，因当前无 docs/CLI/registry 消费方提出需求，属推测性扩展，待真实需求出现再加。
2. **docs 元数据驱动目录** — ✅ 已实施。[component-catalog.ts](../apps/docs/.vitepress/theme/lib/component-catalog.ts) 已 `import { COMPONENT_REGISTRY } from '../../../../../packages/shared/src/index'`，从中派生 `name/title/description/category/status/replacement`；仅本地保留展示性配置（`hiddenDocsEntries`/`slugAliases`/`blockEntries`/`categoryLabels` 本地化标签）。
3. **registry validate 校验 docs** — ✅ 已实施且超出方案。[validate-registry.ts](../packages/registry/scripts/validate-registry.ts) 的 `validateDocsCoverage()` 已做**双向校验**：每个 `COMPONENT_REGISTRY` 条目在中/en 两语言都必有对应页，且每个 docs 页必映射回注册表（带 `kanban→kanban-board` 别名、`input-adornment` 豁免）。比方案原文「校验示例引用」更强。

**残留缺口（已拆为独立任务，不再属于本项）**

`apps/docs/.vitepress/config.ts` 的 sidebar 仍手工维护组件清单，且分类方案（`Basic/Layout/Form/Data Display/Navigation`）与 shared 的 `category`（`action/form/data-display/navigation/feedback/overlay/...`）有意分歧——例如 `button` 在 shared 为 `action`、在 sidebar 为 `Basic`。此为 docs 内部展示重构，与 registry 无关，已拆至下方「独立任务：docs sidebar 元数据生成」。`tags`/`experimental` 等新字段待真实需求出现再加。
...
3. docs 组件目录由 shared 元数据驱动（✅ 已完成，见 Item #11；sidebar 重构见下方独立任务）。

## 独立任务（从 Item #11 拆出，与 registry 无关）

### docs sidebar 元数据生成

**问题**

`apps/docs/.vitepress/config.ts` 的 sidebar 手工维护组件清单，分类方案（`Basic/Layout/Form/Data Display/Navigation`）与 shared `category`（`action/form/data-display/navigation/feedback/overlay/...`）有意分歧。新增组件需同时改 `packages/shared/src/components.ts` 和 `config.ts` 两处，sidebar 缺页不会被 `validateDocsCoverage` 捕获（它只校验页面存在，不校验 sidebar 条目）。

**范围**

- 从 `COMPONENT_REGISTRY` 生成 sidebar 组件分组，或在 shared 增加 `sidebarGroup` 字段供 docs 消费。
- 保留 docs 侧对分组顺序、显示标签、隐藏项的编辑性 override。
- 不强制统一 sidebar 分类与 shared `category`——两者可有意分歧，但分歧应可被审计。

**验收**

- 新增组件只需改 shared 元数据，sidebar 自动出现新条目。
- 新增 sidebar 缺项有 CI 校验（扩展 `validateDocsCoverage` 或新增 sidebar 覆盖校验）。

**优先级**：P3（纯 docs 维护体验优化，不影响分发链路正确性）。
