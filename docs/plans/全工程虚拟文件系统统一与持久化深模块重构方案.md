...
> - 2026-08-19（架构强化与审查修订）：
>   1. **契约固化**：将 `FileSystemAdapter` 中的 `lstat` 与 `mkdtemp` 提升为必选核心契约，消除 Caller 防御性分支与运行时类型盲区；
>   2. **零生产依赖与打包隔离**：确立 `packages/shared` 维持零 runtime 依赖原则，`DiskFileSystemAdapter` 直接基于 Node 22+ 原生 `node:fs/promises` 实现，且 `packages/shared/src/index.ts` 严禁导出 Node 原生模块，强制通过子路径 `brutx-shared-vue/fs` 隔离；
>   3. **上下文聚合**：明确 `AuditLogStorage` 作为 `ProjectContext` 的受管状态域，统一基于异步行迭代器实现内存沙箱与物理磁盘 100% 行为一致的流式审计解析；
>   4. **清理范围全覆盖**：扩充 CLI 双轨适配器与 `fs-extra` 直调清除矩阵，将 `commands/create.ts`、`commands/init.ts`、`lib/project.ts`、`lib/services/init-service.ts` 等全部模块纳入改造清单；
>   5. **AST 安全注入策略**：明确 `BarrelManager` 采用“AST 语法定位 + 源码切片插入”策略，禁止使用 `ts.createPrinter` 全量重写，确保 `packages/ui/src/index.ts` 既有详尽注释、单引号规范与代码风格零破损；
>   6. **规范对齐**：按照 `AGENTS.md` 要求明确开发阶段最小化测试命令与合并门禁验证链条。
