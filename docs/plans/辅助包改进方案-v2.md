...
> 状态：done
> 日期：2026-07-12
> 修订记录：2026-08-11 落地核查确认 —— P0-1/P0-2/P0-3（代码）/P0-4（按方案降级）/P1-5~8/P2-11 全部落地（commit 佐证），P2-9/10 按方案暂缓且轻量替代已落地；唯一缺口 P0-3 的 `name@version` 文档说明页待补

本文是 [v1 方案](../archive/2026/辅助包改进方案-v1.md) 的延续，而非替代。v1 的 P0/P1 大部分已落地，本文基于对三包源码的审计，提出新方向与深化方向。
...
在 [RELEASE.md](../guides/RELEASE.md#breaking-change-迁移文档规范) 中规范化 breaking change 迁移文档模板：
...
BrutxUI 是 copy-paste 式组件库（核心理念见 [README-en.md](../README-en.md)「copy-paste first」），把组件源码复制到每个子包与 monorepo 复用精神部分冲突。`shadcn/ui` 同类产品也不一等支持 monorepo——这是该品类的共识。
