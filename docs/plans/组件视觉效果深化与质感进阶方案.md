...
BrutxUI 在前期演进中，已经通过 R1–R8 视觉规则确立了坚实的基线：统一粗边框（`border-3 border-brutal`）、硬投影（`shadow-brutal`）、盖影语义按压位移（`brutalPress`）、焦点指示环（`FOCUS_RING_CLASSES`）以及 Subtle 浅色衍生背景。
...
| **粗野磨砂 (Frosted)** | 工业磨砂有机玻璃控制盖板 | `backdrop-blur-md bg-brutal-bg/85 border-3 border-brutal shadow-brutal` | 半透明底板透出下层线条与色块，硬边框与硬阴影保持实体感 | 悬浮顶栏（Header）、浮动工具条、DropdownMenu |
...
- **阴影定义**：在 `packages/shared/src/design-tokens.ts` 中注册 `SHADOW_DEFINITIONS`：
  ```typescript
  {
      themeVar: '--shadow-brutal-stacked',
      build: l => `2px 2px 0 0 var(--brutal-border-color, ${l.borderColor}), 4px 4px 0 0 var(--brutal-primary, ${l.primary}), 6px 6px 0 0 var(--brutal-border-color, ${l.borderColor})`,
  }
  ```
- **盖影物理位移等值契约（深度推导）**：
  - 新粗野主义的按压反馈哲学是“**实体按压下沉，刚好落回右下角盖灭自己的影子**”。
  - 普通按钮阴影偏移 4px $\rightarrow$ 按压位移 4px（`brutalPress`）；
  - **Stacked 按钮最外层投影为 6px**，其按压反馈必须精准派生为 6px 位移与去影。
  - 在 `packages/ui/src/lib/brutal-interaction-variants.ts` 中明确派生并导出：
    ```typescript
    const pressStackedX = 'active:translate-x-[6px]'
    const pressStackedY = 'active:translate-y-[6px]'
    export const brutalPressStacked = `${pressStackedX} ${pressStackedY} active:shadow-none`
    ```
  - 严禁出现“阴影 6px 但只移动 4px”或“移动 6px 但残留 2px 阴影”的物理错位。
...
- **阴影定义**：
  ```typescript
  {
      ...
      build: l => `inset 2px 2px 0 0 var(--brutal-border-color, ${l.borderColor})`,
  }
  ```
- **实体装配隐喻**：
...
```mermaid
flowchart TD
    A[用户交互事件 click / touch] --> B{是否处于浏览器环境 & 未静音?}
    B -- 否 --> C[空操作 NOOP / SSR 降级]
    B -- 是 --> D{AudioContext 单例是否存在?}
    D -- 否 --> E[懒加载实例化 AudioContext]
    D -- 是 --> F{state === 'suspended'?}
    F -- 是 --> G[调用 audioCtx.resume()]
    F -- 否 --> H[创建 OscillatorNode + GainNode]
    G --> H
    E --> H
    H --> I[实时生成 25ms 方波衰减 / 白噪声]
    I --> J[播放完毕自动 disconnect 释放节点]
```

- **零外部资源依赖与技术防线**：
  - 严禁引入任何 MP3/WAV 外部音频文件；
  - **Autoplay 策略防御**：禁止在模块加载或 setup 阶段实例化 `AudioContext`，必须在**用户交互触发时懒加载单例**，若处于 `suspended` 状态则先执行 `.resume()`；
  - **实例上限保护**：全库共享单一 `AudioContext` 实例，避免超出浏览器并发上下文上限；
  - **SSR / Node.js 兼容**：在 Nuxt、Vite SSR 或 Vitest 测试环境下（`typeof window === 'undefined'`），自动静默降级为空操作（NOOP）；
  - **声音配方**：
    - **Click (按键音)**：方波 800Hz 快速线性降频至 120Hz，25ms 快速衰减；
    - **Snap (开关音)**：白噪声脉冲 12ms（模拟工控继电器吸合）；
    - **Beep (提示音)**：正弦波 1200Hz，35ms（8-bit 复古蜂鸣）。
- **统一落位与默认静音**：
  - 在 `packages/ui/src/composables/useBrutalHaptics.ts` 中封装导出；
  - 默认完全静音，需通过配置显式开启（`sound: true`）。
...
- **组件 API 规范**：
  ```html
  <BrutalShape name="star-8" size="32" color="var(--brutal-accent)" stroke="black" stroke-width="3" />
  ```
...
- **复合组件设计原则**：
  ...
  - 左侧：3 色微型实心指示方块（红 `#EF476F` / 黄 `#FFE66D` / 绿 `#22c55e`，`w-3 h-3 border-2 border-brutal`），打上 `aria-hidden="true"`；
...
| 组件 | 当前视觉表现 | 专项升级内容 | 涉及文件 |
| :--- | :--- | :--- | :--- |
| **Kbd** | 普通小圆角高亮块 | 1. 引入 3D 机械键帽厚度（`border-b-4` 侧面）<br>2. 按压时下沉消除侧面（`active:border-b-0 active:translate-y-1`）<br>3. 增设工控背光键盘（Backlit）色调变体 | `kbd-variants.ts`<br>`Kbd.vue` |
...
| **Skeleton** | 简单实色 `animate-pulse` | 1. 增设 45° 粗实黑灰斜向扫描线脉冲（`pattern-scanlines`）<br>2. 增设 ASCII 终端块（`██████`）闪烁骨架模式 | `skeleton-variants.ts`<br>`Skeleton.vue` |
...
| **DataTable** | 普通表格网格 | 1. 表头支持斜向条纹（Hatch fill）与点阵<br>2. 选中行高亮应用荧光色块与粗黑框选<br>3. 空状态展示蓝图 HUD 标尺与四角十字准星 | `table-variants.ts`<br>`DataTable.vue` |
...
| **Dialog / Sheet** | 纯色卡片居中弹出 | 1. 遮罩层支持网格点阵滤镜（`overlay-pattern`）<br>2. 弹窗顶栏支持工控窗口控制块与标题<br>3. 入场支持百叶窗（Shutter）机械展开 | `dialog-variants.ts`<br>`DialogContent.vue` |
...
规划面向 Vue 3 生态的 **BrutxUI 粗野主义图表主题系统**（适配 ECharts / 轻量 SVG 渲染器）：

- **粗野主义图表色彩令牌**：
  在 `packages/shared/src/design-tokens.ts` 中扩展图表颜色族（`--chart-1` ~ `--chart-5`）：
  - `--color-chart-1`: `#FF6B6B` (Coral)
  - `--color-chart-2`: `#4ECDC4` (Teal)
  - `--color-chart-3`: `#FFE66D` (Yellow)
  - `--color-chart-4`: `#22c55e` (Green)
  - `--color-chart-5`: `#4A90D9` (Blue)
- **视觉图表法则**：
...
| **物理位移错位** | 彩虹阴影与按压位移距离不一致产生“残留鬼影” | 1. 严格遵守“位移距离 = 最外层阴影偏移”的盖影物理等值契约（6px 投影必配 `brutalPressStacked` 6px 位移与去影）。 |
| **Web Audio 报错** | 页面加载立即初始化触发 Autoplay Policy 警告，或在 SSR 崩溃 | 1. 严禁顶层 `new AudioContext()`；<br>2. 采用用户交互触发的懒加载单例模式；<br>3. `typeof window === 'undefined'` 时静默降级为 NOOP；<br>4. 全局默认静音（opt-in）。 |
| **无障碍合规性** | 装饰字符被读屏软件误读、动效造成前庭功能障碍 | 1. 所有纯装饰符号一律打上 `aria-hidden="true"`；<br>2. 全面支持 `prefers-reduced-motion` 立即瞬时复位；<br>3. 保证装饰层上方前景色文本满足 WCAG AA（4.5:1）。 |
| **Tailwind v4 契约** | 类名动态拼接导致 `@source` 静态扫描漏类 | 1. 所有 CVA 类名声明严格使用完整字面量常量，严禁类名内部 `${...}` 插值；<br>2. 工具类使用 `@utility` 声明，确保变体修饰符正常派生。 |
| **多包与 CLI 漂移** | `packages/ui` 新增阴影/工具类后 CLI 消费方缺失 | 1. 阴影统一在 `design-tokens.ts` 注册，由 `prebuild:tokens` 自动同步至 CLI `brutalist.css`；<br>2. 门禁脚本 `check-brutalist-tokens.ts` 自动拦截跨包令牌差异。 |
...
│  - design-tokens.ts 扩展 stacked / inset 阴影派生与图表色板令牌        │
│  - styles.css 注入 @utility bg-pattern-* 与 scrollbar-brutal 工具类     │
│  - 运行 pnpm --filter brutx-ui-vue prebuild:tokens 重新生成            │
│  - 实现 composables/useBrutalHaptics.ts (SSR 安全的 Web Audio 引擎)     │
│  - 运行 pnpm check:tokens / pnpm check:deprecated 验证底座             │
├─────────────────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────────────────┤
│ 阶段四 (P3): 场景化区块与数据可视化 (Data Visualization & Sections)    │
│  - BrutalistHero / PricingSection 融入 BrutalShape 与工控版式          │
│  - 文档站发布 Neobrutalism Chart 数据可视化设计范式与 ECharts 主题包   │
└─────────────────────────────────────────────────────────────────────────┘
```
