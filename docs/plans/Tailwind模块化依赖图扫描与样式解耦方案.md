...
> 方案类型：CLI 样式架构重构与 Tailwind v4 编译体系深化
> 状态：**active**
> 日期：2026-08-29
> 关联文档：[CLI样式自动生成与单一信源治理方案](CLI样式自动生成与单一信源治理方案.md)；[CLI声明式诊断巡检与自愈引擎方案](CLI声明式诊断巡检与自愈引擎方案.md)；[全工程虚拟文件系统统一与持久化深模块重构方案](全工程虚拟文件系统统一与持久化深模块重构方案.md)；[TAILWIND_V4_MECHANISMS.md](../guides/TAILWIND_V4_MECHANISMS.md)
> 修订记录：
> - 2026-08-29：方案定稿。确立基于 Tailwind v4 原生 `@import` 机制的独立 Token 解耦模式（`tailwind.tokensFile`）、CSS 依赖有向无环图扫描引擎（`CssDependencyGraphEngine`）、相对路径/别名自适应解析器与层叠放置顺序（Cascade Ordering）保护机制。
> - 2026-08-30：完善技术细节。补充 CSS 块注释预剥离（防止 `/* @import */` 幽灵 404 误报）、扩展 `@import` 正则兼容 `layer()` 与媒体查询修饰符、接入 ProjectContext 别名解析器、确立双模式迁移时旧内联 Marker 块清理契约与 Path Traversal 安全门禁。
