...
- **文件**：[DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L699-L705)
- **类别**：逻辑错误 / DOM 绑定
...
- **文件**：[useToast.ts](../../packages/ui/src/composables/useToast.ts#L97-L115) + [Toast.vue](../../packages/ui/src/components/toast/Toast.vue#L59-L89)
- **类别**：竞态条件 / 逻辑错误
...
- **文件**：[Toast.vue](../../packages/ui/src/components/toast/Toast.vue#L74-L82)
- **类别**：逻辑错误
...
- **文件**：[Counter.vue](../../packages/ui/src/components/counter/Counter.vue#L134-L145)
- **类别**：逻辑错误
...
- **文件**：[Upload.vue](../../packages/ui/src/components/upload/Upload.vue#L153-L160)
- **类别**：竞态条件 / 文件上传
...
- **文件**：[FormControl.vue](../../packages/ui/src/components/form/FormControl.vue#L24-L31)
- **类别**：可访问性 / 逻辑错误
...
- **文件**：[FormWizard.vue](../../packages/ui/src/components/form/FormWizard.vue#L59-L99)
- **类别**：逻辑错误（状态转换）
...
- **文件**：[useKanban.ts](../../packages/ui/src/composables/useKanban.ts#L49-L66)
- **类别**：竞态条件 / 生命周期
...
- **文件**：[useDialog.ts](../../packages/ui/src/composables/useDialog.ts#L19-L40)
- **类别**：资源泄漏 / 生命周期
...
- **文件**：[useStepper.ts](../../packages/ui/src/composables/useStepper.ts#L33-L71)
- **类别**：逻辑错误 / 可访问性
...
- **文件**：[SubMenu.vue](../../packages/ui/src/components/menu/SubMenu.vue#L130-L134)
- **类别**：事件处理 / UX
...
- **文件**：[functional.ts](../../packages/ui/src/components/dialog/functional.ts#L246)
- **类别**：i18n / 硬编码
...
- **文件**：[Tour.vue](../../packages/ui/src/components/tour/Tour.vue#L321-L363)
- **类别**：可访问性 / 事件处理
...
- **文件**：[GlitchText.vue](../../packages/ui/src/components/glitch-text/GlitchText.vue#L35-L44)
- **类别**：内存泄漏 / 动画
...
- **文件**：[TreeView.vue](../../packages/ui/src/components/tree-view/TreeView.vue#L93-L110)
- **类别**：响应式 / v-model
...
- **文件**：[DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L448-L624)
- **类别**：可访问性
...
- **文件**：[DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L91-L95)
- **类别**：逻辑错误 / 响应式
...
| # | 文件 | 描述 |
|---|------|------|
| L-1 | [useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L297-L308) | `initSize` 赋值 `sizeRafId = requestAnimationFrame(...)` 前未取消前一个 rAF，`onBeforeUnmount` 仅能取消最新 ID |
| L-2 | [useTheme.ts](../../packages/ui/src/composables/useTheme.ts#L184-L206) | 单例 fallback 的 `matchMedia` listener 无组件级清理，仅靠 `beforeunload` 移除 |
| L-3 | [render-imperative.ts](../../packages/ui/src/lib/render-imperative.ts#L67-L72) | `destroy()` 中 `setTimeout(removeContainer, delay)` 未跟踪，页面导航后可能残留空 div（`container.remove()` 对已移除节点是 no-op，影响极小） |

### 竞态条件

| # | 文件 | 描述 |
|---|------|------|
| L-4 | [Tour.vue](../../packages/ui/src/components/tour/Tour.vue#L225-L358) | 快速点击 Next 时多个 `updatePosition()` 并发，无取消机制，可能闪烁错误步骤的高亮位置 |
| L-5 | [useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L266-L285) | `handleClose` 无并发守卫，快速触发关闭可导致 `beforeClose` 并发执行（重复 API 调用） |

### 逻辑错误

| # | 文件 | 描述 |
|---|------|------|
| L-6 | [Input.vue](../../packages/ui/src/components/input/Input.vue#L228) / [Textarea.vue](../../packages/ui/src/components/textarea/Textarea.vue#L92) | `compositionend` 仅设 `isComposing = false` 但不 emit 最终值，依赖后续 `input` 事件同步（现代浏览器通常触发，但边缘场景可能丢失） |
| L-7 | [Cascader.vue](../../packages/ui/src/components/cascader/Cascader.vue#L381) | 预选值未找到时 `Math.max(0, -1)` 设 `activeItemIndex` 为 0，错误高亮首项 |
| L-8 | [InfiniteScroll.vue](../../packages/ui/src/components/infinite-scroll/InfiniteScroll.vue#L99-L123) | `watch(disabled)` 从禁用转启用时不检查 `immediate`，与 `onMounted` 行为不一致 |
| L-9 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L247-L257) | `warnedColumns` Set 永不重置，列被动态移除再添加后警告不重现 |
| L-10 | [date.ts](../../packages/ui/src/lib/date.ts#L40) | `formatDate` 支持 `WW`（周数）token，但 `parseFormattedDate` 不支持，格式化/解析不对称 |

### 可访问性

| # | 文件 | 描述 |
|---|------|------|
| L-11 | [HardcoreInput.vue](../../packages/ui/src/components/hardcore-input/HardcoreInput.vue#L103-L117) | 无 IME 组合处理（`compositionstart`/`compositionend`），CJK 输入时每个中间状态都 emit `update:modelValue` 并可能触发验证 |
| L-12 | [SelectTrigger.vue](../../packages/ui/src/components/select/SelectTrigger.vue#L82-L101) | 清除按钮为 `<span role="button" tabindex="0">` 嵌套在 `<button>` 内，产生额外 tab stop |
| L-13 | [Rate.vue](../../packages/ui/src/components/rate/Rate.vue#L86-L102) | 无 Home/End 键支持；值未变化时仍 emit `update:modelValue`（冗余事件） |
| L-14 | [MenuItem.vue](../../packages/ui/src/components/menu/MenuItem.vue#L77) / [SubMenu.vue](../../packages/ui/src/components/menu/SubMenu.vue#L206) | `@keydown.space` 无 `.prevent`，按 Space 激活菜单项时页面滚动 |

### 类型安全

| # | 文件 | 描述 |
|---|------|------|
| L-15 | [Combobox.vue](../../packages/ui/src/components/combobox/Combobox.vue#L84-L149) | `props.modelValue as string[]` 不安全：若 `multiple=true` 但 `modelValue` 为 string，`String.prototype.includes` 做子串匹配而非精确匹配 |
| L-16 | [TreeSelectNode.vue](../../packages/ui/src/components/tree-select/TreeSelectNode.vue#L96) | `closest()` 返回 `Element \| null` 但直接 `as HTMLElement`，未做 `instanceof` 守卫 |

### v-model / 响应式

| # | 文件 | 描述 |
|---|------|------|
| L-17 | [TreeSelect.vue](../../packages/ui/src/components/tree-select/TreeSelect.vue#L94-L100) | `open` computed setter 在受控模式下仍更新 `internalOpen`，与 Combobox/Cascader 的 `if (props.open === undefined)` 守卫不一致 |
| L-18 | [useCarousel.ts](../../packages/ui/src/composables/useCarousel.ts#L89-L103) | `onMounted` 在 `emblaApi.value` 为 null 时 early return 且无 `watch` 恢复路径，条件渲染（`v-if`）的轮播可能永不初始化 |
| L-19 | [useCarouselEnhanced.ts](../../packages/ui/src/composables/useCarouselEnhanced.ts#L40-L73) | 进度定时器不检查 autoplay 是否实际运行，`prefersReducedMotion` 时仍可能推进 `autoplayProgress` |

### 性能

| # | 文件 | 描述 |
|---|------|------|
| L-20 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L699-L705) | `getSpanMethodResult` 每个单元格每帧调用 6 次（3× in `:style`, 3× in `v-show`） |
| L-21 | [Upload.vue](../../packages/ui/src/components/upload/Upload.vue#L72-L74) | `watch(fileList, ..., { deep: true })` 在上传进度更新时频繁触发并替换数组，大文件列表产生不必要重渲染 |

### 代码质量 / 硬编码

| # | 文件 | 描述 |
|---|------|------|
| L-22 | [useGlitchEffect.ts](../../packages/ui/src/composables/useGlitchEffect.ts#L13-L14) | 本地常量 `MIN_AUTOPLAY_INTERVAL_MS = 100` 与 `defaults.ts` 的 `GLITCH_MIN_INTERVAL_MS = 50` 分歧 |
| L-23 | [useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L266-L285) | 用 `function.length` 区分 callback/Promise 模式不可靠：`(done = () => {}) => ...` 会被误判为 Promise 模式 |
| L-24 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L130-L475) | 魔法数字：`150`（列宽回退）、`'40px'`/`'48px'`（展开/选择列宽）、`48`（行高回退） |
