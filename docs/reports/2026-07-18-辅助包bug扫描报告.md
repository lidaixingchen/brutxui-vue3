...
> **对照基线**：[auxiliary-packages-bug-scan-2026-07-12.md](2026-07-12-辅助包bug扫描报告.md)（共 34 项 bug：C1-C4、M1-M11、L1-L19）
...
| Bug | 状态 | 修复方式 | 验证 |
|-----|------|----------|------|
| C1 `tabs` 缺失 `Tabs.vue` | ✅ 已修复 | `scan-component-files.ts` 递归扫描组件目录，自动发现所有 `.vue`/`.ts`/`.css` 文件 | [packages/shared/src/scan-component-files.ts](../../packages/shared/src/scan-component-files.ts) `listComponentFiles` |
...
| Bug | 状态 | 修复方式 | 验证 |
|-----|------|----------|------|
| M1 `matchFileByPath` 假阳性匹配 | ✅ 已修复 | 改为前缀剥离后严格相等比较 | [packages/cli/src/lib/services/diff-service.ts](../../packages/cli/src/lib/services/diff-service.ts#L19-L29) L19-L29 |
| M2 `--path` 模式 snippets 写错目录 | ✅ 已修复 | snippets 检测和写入使用 `targetCwd` | [packages/cli/src/commands/add.ts](../../packages/cli/src/commands/add.ts) |
| M3 `create` 命令注入 | ✅ 已修复 | 添加 `PROJECT_NAME_PATTERN = /^[a-zA-Z0-9._-]+$/` 校验 | [packages/cli/src/commands/create.ts](../../packages/cli/src/commands/create.ts) |
| M4 Nuxt 配置写入失败未回滚 | ✅ 已修复 | `initializeProjectFiles` 在 `nuxt.status === 'write-failed'` 时抛异常触发 `rollback` | [packages/cli/src/lib/services/init-service.ts](../../packages/cli/src/lib/services/init-service.ts#L281-L283) L281-L283 |
| M5 `validateComponents` 阻止自定义 registry | ✅ 已修复 | `registryOverride` 存在时跳过白名单校验 | [packages/cli/src/commands/add.ts](../../packages/cli/src/commands/add.ts#L61-L63) L61-L63 |
| M6 `rewriteImports` 正则过度匹配 | ✅ 已修复 | `./` 重写仅在 `<script>` 块内执行 | [packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L469-L477) L469-L477 |
| M7 Locale 文件未通过 `rewriteImports` | ✅ 已修复 | `run()` 中 locale 文件调用 `rewriteImports` | [packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L926) L926 |
| M8 `buildRegistryItem` 不处理 locale 传递依赖 | ✅ 已修复 | 添加 locale 文件读取和依赖提取 | [packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L703-L717) L703-L717 |
| M9 `computeSourceHash` 不动点迭代顺序 | ✅ 已修复 | 合并 composables 和 locales 为单一 while 循环 | [packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L392-L411) L392-L411 |
| M10 `COMPONENTS_BY_CATEGORY` 含 block | ✅ 已修复 | `createComponentsByCategory` 跳过 `kind === 'block'` | [packages/shared/src/component-metadata.ts](../../packages/shared/src/component-metadata.ts) |
| M11 `getComponentsByCategory` 返回可变引用 | ✅ 已修复 | 返回 `[...COMPONENTS_BY_CATEGORY[category]]` 浅拷贝 | [packages/shared/src/component-metadata.ts](../../packages/shared/src/component-metadata.ts) |
...
| L17 `utils.ts` 传递依赖 | ✅ 已修复 | 排除后仍读取内容并提取 `extractDeps`，仅跳过 `parts.push` | [packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L418-L424) L418-L424 |
...
- [packages/cli/src/lib/cache.ts](../../packages/cli/src/lib/cache.ts#L52-L54) L52-L54（`isCacheDisabled()`）
- [packages/cli/src/lib/registry.ts](../../packages/cli/src/lib/registry.ts#L115) L115（`getItem`）
- [packages/cli/src/lib/registry.ts](../../packages/cli/src/lib/registry.ts#L356) L356（`resolveDeps`）
...
- **文件**：[packages/cli/src/lib/project.ts](../../packages/cli/src/lib/project.ts#L276-L279) L276-L279
- **类别**：别名解析 / 逻辑不一致
- **问题描述**：
  ...
  **触发链路**（默认配置下不触发）：
  ...
  2. CLI 安装 `lib/foo` 文件时，[add-service.ts L98-L106](../../packages/cli/src/lib/services/add-service.ts#L98-L106) 将其写入 `path.dirname(resolveAliasPath('@/utils'))` = `src/`
...
- **文件**：[packages/registry/scripts/validate-utils.ts](../../packages/registry/scripts/validate-utils.ts#L161-L164) L161-L164
- **类别**：死代码 / 数据一致性
- **问题描述**：
...
- **文件**：[packages/cli/src/lib/package-manager.ts](../../packages/cli/src/lib/package-manager.ts#L12-L14) L12-L14
- **类别**：输入处理 / 兼容性
- **问题描述**：
...
- **文件**：[packages/cli/src/commands/doctor.ts](../../packages/cli/src/commands/doctor.ts#L631-L655) L631-L655
- **类别**：性能 / 资源浪费
- **问题描述**：
...
- **文件**：[packages/cli/src/lib/project.ts](../../packages/cli/src/lib/project.ts#L203-L207) L203-L207
- **类别**：功能限制
- **问题描述**：
...
- **文件**：[packages/cli/src/lib/project.ts](../../packages/cli/src/lib/project.ts#L236-L248) L236-L248
- **类别**：已知限制 / 文档化
- **问题描述**：
...
- **位置**：[packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L920) L920
- **观察**：`en.ts` 不在发布列表中。
- **评估**：`useLocale.ts` 仅导入 `zh-CN` 和 `types`，`en.ts` 仅被测试文件导入（41 处 `.test.ts`）。registry 不发布 `en.ts` 是正确的——它不是运行时依赖。

### O2：`BRUTX_NO_CACHE` 环境变量保留

- **位置**：[packages/cli/src/lib/cache.ts](../../packages/cli/src/lib/cache.ts#L52-L54) L52-L54、[packages/cli/src/lib/registry.ts](../../packages/cli/src/lib/registry.ts#L115) L115
...
- **位置**：[packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L81) L81（`SCAN_LIB_EXCLUDE`）、L119（`LIB_FILE_EXCLUDE`）
- **观察**：两个常量都排除 `utils.ts`。
...
- **位置**：[packages/registry/scripts/build-registry.ts](../../packages/registry/scripts/build-registry.ts#L791) L791
