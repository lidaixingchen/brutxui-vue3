...
> **对照基线**：[ui-bug-scan-2026-07-11.md](2026-07-11-ui界面bug扫描报告.md)
...
| 既有 ID | 文件 | 修复方式摘要 |
|---------|------|--------------|
| C-1 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L133-L141) | `spanMethod` 结果预计算到 `Map` 缓存（`getCellSpan`），模板改为 `:rowspan` / `:colspan` HTML 属性绑定（L718-719） |
| H-2 | [Toast.vue](../../packages/ui/src/components/toast/Toast.vue#L75) | `resumeTimer()` 增加 `if (!props.duration) return` 早返回 |
| H-3 | [Counter.vue](../../packages/ui/src/components/counter/Counter.vue#L134-L145) | 引入独立隐藏 `measureRef` span 显示 `finalDisplayValue` 用于自然宽度测量；`constrainedWidth` 改用 `root.parentElement?.clientWidth ?? root.clientWidth` |
| H-4 | [Upload.vue](../../packages/ui/src/components/upload/Upload.vue#L157-L159) | `retryUpload` 增加 `if (file.status === 'uploading') file.abortController?.abort()`；`doUpload` 增加 `settled` 标志与 `isCancelled` 守卫防止重复 emit |
| M-1 | [FormControl.vue](../../packages/ui/src/components/form/FormControl.vue#L25-L34) | 改用 reka-ui `Primitive` + `as-child` 合并 `id` / `aria-describedby` / `aria-invalid` 到 slotted 子元素 |
| M-3 | [useKanban.ts](../../packages/ui/src/composables/useKanban.ts#L52-L65) | `onDragStart` 与 `onDragEnd` 均增加 `if (dragEndRafId !== null) cancelAnimationFrame(dragEndRafId)` 守卫 |
| M-4 | [useDialog.ts](../../packages/ui/src/composables/useDialog.ts#L20-L22) | `show()` 先调用 `currentInstance.close()` 关闭前一个实例再创建新实例 |
| M-5 | [useStepper.ts](../../packages/ui/src/composables/useStepper.ts#L33-L71) | `goToStep` 增加 `force` 参数；Home/End 调用 `goToStep(0, true)` / `goToStep(totalSteps - 1, true)` 跳过线性检查 |
| M-6 | [SubMenu.vue](../../packages/ui/src/components/menu/SubMenu.vue#L233) | 水平模式下用外层 `<div class="absolute top-full left-0 pt-1.5">` padding 包裹，替代 `<ul>` 的 `mt-1.5` margin，保持悬停区域连续 |
| M-8 | [Tour.vue](../../packages/ui/src/components/tour/Tour.vue#L329-L335) | `handleKeyDown` 增加 `target.tagName` 检查，INPUT/TEXTAREA/isContentEditable 元素直接 return |
| M-9 | [GlitchText.vue](../../packages/ui/src/components/glitch-text/GlitchText.vue#L5) | 统一从 `@/lib/defaults` 导入 `DEFAULT_AUTOPLAY_INTERVAL_MS` / `GLITCH_AUTOPLAY_ACTIVE_DURATION_MS` / `GLITCH_MIN_INTERVAL_MS`；`clearTimeout(autoplayStopTimer.value)` 在赋新值前调用（L40-42） |
| M-10 | [TreeView.vue](../../packages/ui/src/components/tree-view/TreeView.vue#L107-L114) | `emitNodesUpdate` 通过 `getCurrentInstance()?.vnode.props?.['onUpdate:nodes']` 检测监听器存在性，仅当存在监听器时才设 `isUpdatingInternally` 标志 |
| M-11 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L448-L624) | 可排序 `<th>` 增加 `@keydown.space.prevent="handleSort(column.id)"` |
| M-12 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L91-L95) | `watch(expandRowKeys)` 改为 `expandedRowKeys.value = newKeys ? new Set(newKeys) : new Set()`，正确处理 `undefined` |
| L-4 | [Tour.vue](../../packages/ui/src/components/tour/Tour.vue#L225-L245) | `updatePosition` 引入 `positionRequestId` 自增 ID + `if (requestId !== positionRequestId) return` 取消过期异步更新 |
| L-6 | [Input.vue](../../packages/ui/src/components/input/Input.vue#L228) | `compositionend` 现在会 emit 最终值 |
| L-8 | [InfiniteScroll.vue](../../packages/ui/src/components/infinite-scroll/InfiniteScroll.vue#L102-L115) | `watch(disabled)` 启用分支增加 `if (props.immediate || observerResult === 'unsupported')` 检查 |
| L-11 | [HardcoreInput.vue](../../packages/ui/src/components/hardcore-input/HardcoreInput.vue#L192-L193) | 增加 `@compositionstart="isComposing = true"` 与 `@compositionend="onCompositionEnd"` 处理 |
| L-13 | [Rate.vue](../../packages/ui/src/components/rate/Rate.vue#L92-L107) | 增加 Home/End 键处理；增加 `nextValue !== props.modelValue` 守卫避免冗余 emit |
| L-14 | [MenuItem.vue](../../packages/ui/src/components/menu/MenuItem.vue#L77) / [SubMenu.vue](../../packages/ui/src/components/menu/SubMenu.vue#L206) | `@keydown.space` 增加 `.prevent` 修饰符 |
| L-15 | [Combobox.vue](../../packages/ui/src/components/combobox/Combobox.vue#L141-L146) | `isSelected` 增加 `Array.isArray(props.modelValue)` 守卫后再 `.includes()`，避免字符串做子串匹配 |
| L-16 | [TreeSelectNode.vue](../../packages/ui/src/components/tree-select/TreeSelectNode.vue#L91-L100) | `focusParent` 增加 `if (!currentItem) return` null 检查 + `instanceof HTMLElement` 守卫 |
| L-17 | [TreeSelect.vue](../../packages/ui/src/components/tree-select/TreeSelect.vue#L94-L102) | `open` setter 增加 `if (props.open === undefined)` 守卫，受控模式下不更新 `internalOpen` |
| L-18 | [useCarousel.ts](../../packages/ui/src/composables/useCarousel.ts#L109-L112) | 增加 `watch(emblaApi, ...)` 监听 `emblaApi` 后续可用时调用 `initCarousel` 恢复初始化路径 |
| L-19 | [useCarouselEnhanced.ts](../../packages/ui/src/composables/useCarouselEnhanced.ts#L42-L57) | `startProgressTimer` 增加 `if (!toValue(options.autoplay) || prefersReducedMotion.value) return` 守卫 |
| L-20 | [DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L133-L141) | `getCellSpan` 缓存 `spanMethod` 结果到 `Map`，避免每帧重复调用 6 次 |
| L-21 | [Upload.vue](../../packages/ui/src/components/upload/Upload.vue#L72-L74) | `watch(() => props.fileList, ...)` 改为引用监听（无 `deep: true`），避免进度更新频繁触发 |
| L-22 | [useGlitchEffect.ts](../../packages/ui/src/composables/useGlitchEffect.ts#L2) | 统一从 `@/lib/defaults` 导入 `GLITCH_MIN_INTERVAL_MS`，删除本地 `MIN_AUTOPLAY_INTERVAL_MS = 100` |
| L-23 | [useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L270-L280) | `handleClose` 不再使用 `function.length` 区分模式，改为直接 `await opt.beforeClose()` 判断返回值 |
...
- **文件**：[useToast.ts](../../packages/ui/src/composables/useToast.ts#L96-L120) + [Toast.vue](../../packages/ui/src/components/toast/Toast.vue#L59-L90)
- **类别**：竞态条件 / 逻辑错误
...
- **文件**：[FormWizard.vue](../../packages/ui/src/components/form/FormWizard.vue#L59-L66)
- **类别**：逻辑错误（状态转换）
...
- **文件**：[functional.ts](../../packages/ui/src/components/dialog/functional.ts#L296-L301)
- **类别**：i18n / 代码质量
...
- **文件**：[TreeView.vue](../../packages/ui/src/components/tree-view/TreeView.vue#L119-L121)
- **类别**：v-model / 响应式
...
- **文件**：[useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L292-L303)
- **类别**：内存泄漏 / 生命周期
...
- **文件**：[useTheme.ts](../../packages/ui/src/composables/useTheme.ts#L193-L209)
- **类别**：内存泄漏
...
- **文件**：[render-imperative.ts](../../packages/ui/src/lib/render-imperative.ts#L58-L74)
- **类别**：内存泄漏
...
- **文件**：[useDialogEnhanced.ts](../../packages/ui/src/composables/useDialogEnhanced.ts#L270-L280)
- **类别**：竞态条件
...
- **文件**：[Cascader.vue](../../packages/ui/src/components/cascader/Cascader.vue#L381-L382)
- **类别**：逻辑错误
...
- **文件**：[DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L262-L272)
- **类别**：逻辑错误
...
- **文件**：[date.ts](../../packages/ui/src/lib/date.ts#L30-L80)
- **类别**：逻辑错误 / 格式不对称
...
- **文件**：[DataTable.vue](../../packages/ui/src/components/data-table/DataTable.vue#L274-L289)
- **类别**：代码质量
