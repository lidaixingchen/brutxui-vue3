...
> 方案类型：重构 / 流程改造
> 状态：已定稿（分发端点选型：GitHub Release 资产）
> 关联文档：[INFRASTRUCTURE_CLOSURE_PLAN.md](INFRASTRUCTURE_CLOSURE_PLAN.md)、[RELEASE.md](RELEASE.md)
...
1. **反复发生的合并冲突**：产物是"双源驱动"的生成物——
   ...
   > 冲突归因说明（避免误导）：产物并非"每次构建必然不同"。`buildTimestamp` 默认 `null`，仅在注入 `BRUTX_REGISTRY_BUILD_TIMESTAMP` 时变化（[build-registry.ts:1158](packages/registry/scripts/build-registry.ts#L1158)）；SBOM `serialNumber` 是内容哈希派生的**确定性值**（[build-registry.ts:860-875](packages/registry/scripts/build-registry.ts#L860-L875)，注释明确"可复现构建"）。系统性差异来自**源码改动驱动的 integrity 变化**与 **CI 环境注入的 gitCommit/签名**。
...
- **CLI 官方源指向新端点**，零配置用户体验不中断，签名校验链路真正生效；
...
packages/registry/registry/{name}.json + index.json + registry-manifest.json + registry-sbom.json
        ...
        ├─ pre-commit 钩子（ui/src 变更 → 重建 + git add 三类生成文件）
...
- **签名机制已就绪但从未生效**：`signManifestFromEnv`（[build-registry.ts:919-931](packages/registry/scripts/build-registry.ts#L919-L931)）检测到 `BRUTX_REGISTRY_PRIVATE_KEY`/`BRUTX_REGISTRY_KEY_ID` 即自动签名；publish.yml 已注入私钥（[publish.yml:22-23](.github/workflows/publish.yml#L22-L23)）。但当前 main 上已提交的 manifest 为**未签名**状态（`gitCommit/signature/buildTimestamp` 均为 null）——签名回填机制实际从未产出过签名产物。方案落地即让签名在发布时真正生效。
...
- **CLI 验签链路完整**：[signature.ts](packages/cli/src/lib/signature.ts)（`verifyManifestSignature` :149-206、`verifyManifestIntegrityAndSignature` :266-288），内置官方公钥 `OFFICIAL_PUBLIC_KEYS`（[constants.ts:129-136](packages/cli/src/lib/constants.ts#L129-L136)，keyId `official-v1`）。无签名时 CLI 向后兼容跳过验签，有签名时强校验。
...
- **扁平命名，无目录层级**：上传后资产名是 basename（`button.json`、`index.json`…），可寻址 URL 只能是 `releases/latest/download/{name}.json`，**不存在子路径**。CLI 按 `${source}/{name}.json` 拼接（[registry.ts:262](packages/cli/src/lib/registry.ts#L262)），默认源直接指向 `releases/latest/download` 即天然适配。
...
- [ ] **T1 发布流程新增产物上传**（[.github/workflows/publish.yml](.github/workflows/publish.yml)）
  - **将 `gh release create`（:123-130）与新增的 upload 步骤整体前移至 npm publish（:85-121）之前**（tag 触发时 release 尚不存在，create 放最前安全；`concurrency: publish-global, cancel-in-progress: false` 保证串行）。新增 `gh release upload <tag> packages/registry/registry/*.json --clobber`：
    - **扁平命名**：上传后资产名即 basename，可寻址 URL 为 `releases/latest/download/{name}.json`，**无子路径**；CLI 默认源直接指向该目录即可；
    - **upload 步骤需显式声明 `GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}`**（gh 靠环境变量认证，不会自动继承；job 级 env 只有私钥/TURBO）；
    - **`--clobber` 需 gh ≥ 2.3.0**：ubuntu-latest 预装版满足，但属隐式假设，建议加 `gh version` 门控或注释说明；fresh tag 时 `--clobber` 为 no-op，仅 tag 重推/重跑时有意义；
    - **90 个文件单命令上传可行**（每个文件一次 REST 请求，小文件秒级），**不打包**——CLI 按 `{source}/{name}.json` 逐文件拉取，打成 tar 会让单文件拉取全部失效（除非 T2 同步改 CLI 为整包下载解压，本方案不采纳）；
  - 上传前校验产物齐全（`index.json` / `registry-manifest.json` / `registry-sbom.json` 存在、manifest 含 `signature`/`keyId`）；
...
- [ ] **T2 CLI 默认源切换**（[packages/cli/src/lib/constants.ts:106-109](packages/cli/src/lib/constants.ts#L106-L109)）
  ...
  - **`@version` 版本化语法降级**：`resolveVersionedSource`（[registry.ts:504-515](packages/cli/src/lib/registry.ts#L504-L515)）仅匹配 `GITHUB_RAW_URL_PATTERN`（:24），Release 源下显式 `button@v1` 当前会抛 `REGISTRY_VERSION_UNSUPPORTED`。降级语义：对默认源上的 `@version` **忽略版本、按 latest 拉取**（raw 源仍保留版本化能力），同步更新 `error-advice.ts:30-33` 文案与相关测试；
  - **同步更新 CLI 测试**：
...
- [ ] **T3 同步文档**
  ...
  - [docs/RELEASE.md](docs/RELEASE.md)：§"版本发布"门禁说明（:3-9）、§"重新编译注册表"（:8）**、§"Registry manifest 自动签名"（:201-209，描述 T4/T5 将删除的 sign-manifest job 与 main 同步机制）**、:16 产物语义微调。
...
- [ ] **T5 改造 ci.yml**：
  ...
  - **drift 门禁（:75-85）按路径细分**：移除 `packages/registry/registry` 路径的 diff 检查，**保留 `packages/ui/registry-manifest.json` 与 `packages/ui/src/styles.css` 的检查**——这两个 ui 生成文件仍入库跟踪、仍由 prebuild 生成，若一并放开，后续 tag 发布会被（保留的 ui 侧门禁）卡住且失去漂移预警。
...
- [ ] **T11 turbo.json 语义收紧**（[turbo.json](turbo.json)）：
  ...
  - **`test`/`validate` 的 `inputs` 保留 `registry/**`（:57、:75）**——产物是它们的真实输入，删除会在"产物变但源码哈希不变"时缓存误命中；
  - **补齐当前缺失的输入源**：build-registry.ts 实际直读 `ui/registry-manifest.json`、`ui/src/locales/**`、`ui/src/lib/**`（[build-registry.ts:35-38](packages/registry/scripts/build-registry.ts#L35-L38)），但当前 build/validate 的 inputs 均不含这些路径——`validate` 的 inputs 应补齐 `../../packages/ui/registry-manifest.json`、`../../packages/ui/src/locales/**`、`../../packages/ui/src/lib/**`；
  - **Turbo Remote Cache 放大说明**：publish.yml/ci.yml 已启用 Remote Cache（[publish.yml:16-19](.github/workflows/publish.yml#L16-L19)）。缓存键缺失输入源会在跨 commit/跨分支间复用陈旧 validate/test 结果，故上述补齐是正确性的要求，不是可选优化。若确需减小缓存键，替代方案是直接给 `validate`/`test` 设 `cache:false`。
...
- [ ] **T13 verify-build.ts**：产物不入库后其"备份/恢复 `.registry-cache.json`"逻辑不受影响，写回磁盘的产物由 T8 gitignore 覆盖，无需改动；**顺带修正 :11 过时注释**（"serialNumber 每次 build 随机生成" → 实际为内容哈希派生的确定性值，[build-registry.ts:860-875](packages/registry/scripts/build-registry.ts#L860-L875)）；补充注释说明其运行前提
- [ ] **T14 开发文档**：
  ...
  - [AGENTS.md:11](AGENTS.md#L11)：自动生成文件表移除 registry 产物项（:9-10 的 ui manifest/styles.css 与 :50-52 的命令说明**不在**改动范围）。
...
| 幂等性 | ci.yml 特意 `unset GITHUB_SHA` 保证 gitCommit=null 幂等 | 发布场景下 gitCommit=GITHUB_SHA（tag 提交）可接受；签名对 `integrity` 签发、`integrity` 排除 gitCommit/buildTimestamp（[build-registry.ts:246-257](packages/registry/scripts/build-registry.ts#L246-L257)），**签名跨发布稳定**，无需 unset |
...
5. 发布验证：打 `v*` tag → 确认 Release 资产（`releases/latest/download/registry-manifest.json` 可达且含签名）→ `brutx-vue add` 冒烟（新默认源 + 验签）→ 本地 pull 确认无产物冲突 → **验证 ui/registry-manifest.json 与 styles.css 的 ui 侧门禁仍生效**。
...
| `packages/cli/src/lib/registry.ts` | :24（URL 结构解析）、:504-515（@version 降级） | 修改 |
