...
1. **CLI `init` 阶段（创建共享基座）**：
   用户运行 `brutx-vue init` 时，将 brutx 公共 runtime 文件写入隔离的共享目录（默认 `@/components/brutx/shared/`），不再污染用户 `src/composables/`：
   ```
...
1. **建立设计令牌单一数据源**：
   新增 `packages/ui/src/lib/design-tokens.ts`，作为所有设计令牌的唯一来源：
   ```typescript
   export const DESIGN_TOKENS = {
       border: { width: '3px', color: '#000000' },
       shadow: { offsetX: '4px', offsetY: '4px', color: '#000000' },
       radius: '0px',
       pressedOffset: '2px',
       colors: {
   ...
   ```
   - `styles.css` 的 @theme 与 :root 变量从该文件生成（构建时注入）。
   - registry JSON 的 `cssVars` 字段从该文件生成。
   - CSS fallback 值统一引用该文件常量。
...
- 实现 CLI 共享基座（init 写入 + resolveImportAlias 扩展）
