...
- 焦点指示统一用 outline（ring 系已废弃：ring 基于 box-shadow，与 brutal 阴影同属性争用，聚焦瞬间阴影会"跳动"）：统一引用 `@/lib/utils` 的 `FOCUS_OUTLINE_CLASSES`（`focus-visible:outline-2 focus-visible:outline-brutal-ring focus-visible:outline-offset-2`）；focus-within 容器（input/number-input/TagsInput）用同款 `focus-within:` 变体；紧凑控件（data-table ColumnFilter）保留 `outline-1`。**禁止在同一元素上保留任何 `outline-none`**——Tailwind v4 的 `--tw-outline-style` 变量机制会把它扩散为 `none`，抑制焦点 outline 的渲染
