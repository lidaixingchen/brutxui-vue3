...
| 目录 / 文件 | 内容定义 | 生命周期特征 | 维护策略 |
| :--- | :--- | :--- | :--- |
| `index.md` | 全局知识地图与导航索引 | **常青**（Long-lived） | 由归档引擎自动化维护与驱动自愈 |
| `README-en.md` | 根 README 英文镜像版本 | **常青**（Long-lived） | 随根 `README.md` 同步演进维护，保持章节与核心说明 1:1 对齐 |
...
## 六、 方案执行完毕后的收尾与归档 SOP（Agent / 开发者操作指南）

当 Agent 或开发者根据某篇方案（位于 `docs/plans/<domain>/<name>.md`）完成编码与落地任务后，**严禁仅提交代码或留下未流转的半成品文档直接结束**。必须严格依循以下 4 步收尾闭环 SOP，保障工程状态机、文档索引与门禁的绝对一致性：

### 步骤 1：功能验收与针对性测试
- **契约对齐**：对照方案声明的交付清单，确认核心组件、变体逻辑、类型接口与 CLI 指令已全量实现。
- **编写/更新单测**：在对应目录下补充 `*.test.ts`、`*.a11y.test.ts`、`*-keyboard.test.ts` 等测试用例。
- **最小化自检**：
  ```bash
  pnpm --filter <pkg> test <相对测试文件路径>   # 运行针对性单测
  pnpm --filter <pkg> typecheck               # 验证 TS 严格模式无类型错误
  ```

### 步骤 2：一键自动归档与知识地图全景自愈
依据黄金判据（读完不用做 = done），在根目录下直接运行一键归档命令：
```bash
pnpm doc:archive docs/plans/<domain>/<name>.md
```
> [!TIP]
> 引擎将全自动执行：
> 1. **物理文件原子迁移**：由 `docs/plans/<domain>/` 自动按完工年份移入 `docs/archive/YYYY/<domain>/`；
> 2. **Frontmatter 终态固化**：自动流转为 `状态: archived` 并注入当天的 `完工日期: YYYY-MM-DD`（原 `日期` 保持不可变）；
> 3. **双向相对链接拓扑自愈**：自动完成文档内相对路径绝对重算（深度 3 $\to$ 4），并基于全仓倒排索引将外部所有反向引用重定向至新归档路径；
> 4. **知识地图声明式派生**：全量重新投影并排版 [`docs/index.md`](../index.md) 的活跃区与归档区，保证篇数、顿号与句末标点 100% 精准；
> 5. **精准 Git 暂存**：自动将受影响的文件精准 `git add`，不污染工作区。
>
> 亦可将方案 Frontmatter 设为 `状态: done` 后直接运行 `pnpm doc:archive` 批量收割。
...
### 步骤 4：终态无痕审查与规范提交
- **终态无痕审查（Clean Final State）**：
  - 检查全部变更的代码与注释，严禁包含任何被否决的草案、diff 说明、沟通试错痕迹或防御性注释；
  - 代码注释仅解释当前复杂的业务逻辑与边界。
- **Conventional Commits 原子提交**：
  - 检查工作区状态（`git status`），确保无任何未跟踪临时文件；
  - 遵循终端建议的约定式提交规范提交变更（例如：`docs: 归档xxx方案并同步知识地图` 或 `feat(ui): 实现xxx并归档方案`）。

---

### Agent 收尾自检速查清单（Checklist）
...
| **门禁验收** | 全库文档门禁与静态契约门禁是否均全绿？ | `check:docs` 与 `check:contracts` 全绿 |
| **提交规范** | 是否消除了试错痕迹并遵循约定式提交？ | 工作区干净，提交信息标准 |
