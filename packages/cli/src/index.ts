...
program
    ...
    .option('--reporter <type>', 'Reporter format (pretty, github, json, sarif, junit)')
    .option('--ci', 'CI mode (defaults to github reporter when running in CI)', false)
    .option('--fail-on <level>', 'Exit with code 1 on specified issue level (error, warn, drift)', 'error')
    .option('--output-file <path>', 'Write diagnostic report to specified file path')
    .option('--category <category>', 'Filter checks by category (env, config, tailwind, structure, integrity, custom)')
    .option('--rule <ruleId>', 'Run specific diagnostic rule by ID')
    .option('-y, --yes', 'Skip confirmation prompts', false)
    .option('-s, --silent', 'Mute output', false)
