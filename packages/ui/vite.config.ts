...
import { isJsOutputFile, rewriteFlattenedChunkImports } from './src/lib/preserve-modules-paths'
    ...
    return {
        ...
        writeBundle(options) {
            ...
            for (const srcFile of files) {
                ...
                if (isJsOutputFile(relFromNested)) {
                    let content = readFileSync(srcFile, 'utf-8')
                    content = rewriteFlattenedChunkImports(content, destFile, distDir)
            ...
            for (const entry of readdirSync(distDir, { withFileTypes: true })) {
                ...
                if (!isJsOutputFile(entry.name)) continue
                const filePath = join(distDir, entry.name)
                let content = readFileSync(filePath, 'utf-8')
                const rewritten = rewriteFlattenedChunkImports(content, filePath, distDir)
