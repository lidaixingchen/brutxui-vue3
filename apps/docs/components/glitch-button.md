...
> `GlitchButton` 现在是 `Button effect="glitch"` 的兼容 wrapper。新代码建议直接使用 `Button`，以统一 loading、disabled、ARIA 和尺寸变体行为。
...
通过 `Button` 的 `glitchDirection` prop 控制撕裂切缝与错位方向；兼容 `GlitchButton` 时仍可使用旧的 `direction` prop。
...
> **`data-text` 属性**：故障按钮的撕裂效果通过伪元素 `content: attr(data-text)` 复制文本实现，因此需要通过 `data-text` 属性传入与按钮内容一致的文本，否则伪元素复制层为空。这是与 GlitchText（自动从 `text` prop 绑定 `data-text`）的关键差异。
...
| `speed`    | `'slow' \| 'medium' \| 'fast'`                                                             | `'medium'`  | 兼容 prop；对应 `Button` 的 `glitchSpeed` |
| `direction` | `'horizontal' \| 'vertical' \| 'both'`                                                    | `'horizontal'` | 兼容 prop；对应 `Button` 的 `glitchDirection` |
| `trigger`  | `'hover' \| 'click' \| 'autoplay' \| 'none'`                                              | `'hover'`   | 兼容 prop；对应 `Button` 的 `glitchTrigger` |
| `interval` | `number`                                                                                   | `3000`      | 兼容 prop；对应 `Button` 的 `glitchInterval` |
