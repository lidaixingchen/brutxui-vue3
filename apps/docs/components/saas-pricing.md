...
- **键盘操作**：套餐选择按钮支持 Tab 键聚焦，Enter/Space 键触发选择
- **ARIA 属性**：计费周期切换控件自动设置 `aria-pressed` 或 `role="switch"` 状态
- **语义化结构**：使用语义化标题层级展示套餐名称和价格信息
- **功能对比**：功能列表使用清晰的视觉标识（勾选/删除线），建议配合文本说明确保信息传达
